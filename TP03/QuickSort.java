...
class Lista {
  ...
  public void inserirFim(Serie item) {
    ...
    public void quickSort(int esq, int dir) {
      ...
      int i = esq, j = dir;
      Serie pivo = listaSerie[(dir+esq)/2];
      while (i <= j) {
          while ((listaSerie[i].getPais().compareTo(pivo.getPais()) < 0) || (listaSerie[i].getPais().compareTo(pivo.getPais()) == 0 && listaSerie[i].getNome().compareTo(pivo.getNome()) < 0)) {
            i++;
          }
          while ((listaSerie[j].getPais().compareTo(pivo.getPais()) > 0) || (listaSerie[j].getPais().compareTo(pivo.getPais()) == 0 && listaSerie[j].getNome().compareTo(pivo.getNome()) > 0)) {
            j--;
          }
          if (i <= j) {
              swap(i, j);
